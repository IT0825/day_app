...
作成日時は記入した日付より未来を表示することができます。  
例：2021年01月01日01時00分の場合(202101010100)と検索。  
ユーザーでログインする場合はヘッダーのログインボタンをクリックして上部のアドレスとパスワードを記入して入力します。  
ログインするとトップページに遷移されますのでトップページの日報作成ボタンをクリックで日報作成の画面へ遷移されます、題名と本文を入力して確認ボタンをクリックして確認画面へ遷移がされます。  
内容に問題なければ作成ボタンクリックで日報作成できます。  

# 目指した課題解決
私の前職ではEXCELで日報を作成をしてメールで上司へ提出していました。  
日報は毎日提出するので、単純に部下の人数分毎日送信されておりました。  
その一人一人の日報の内容を拝見したり過去の日報を探したりするのはとても大変なことです。  
また、作成する側も毎日日報を作成してメール送信の手間も大変です。  
私のアプリケーションを使用することで、日々の業務の軽減が可能となります。  
作成側は少ない工程で日報を作成ができ、パスワードが漏れることがない限り他者に閲覧されることもございません。  
また、過去の自分を振り返ることも検索機能を使用すれば容易に可能です。  
管理側は、メールでデータの容量を日報で使う必要もなく一つのアプリケーションで全ての社員の日報を管理できます。  
また、検索機能を使用することで、指定した日付に提出された日報のみを表示することもできるため、提出漏れなどの確認も容易です。  
